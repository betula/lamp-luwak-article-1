...
На текущий момент в отрасли множество подходов к организации state management вашего приложения. Существующие решения хорошо задокументированы и имеют большое коммьюнити, но так же имеют и свои недостатки.
...
Экшен - это функция, которая может быть источником уведомлений для подписчиков. Экшен можно вызывать как функцию, но рекоммендованный подход для вызова, использовать функцию `dispatch` библиотеки `lamp-luwak`.
...
Конечно, стоит упомянуть, что библиотека ещё свежая и потому приветствуются любые обнаруженные ошибки.
